        ...
        try:
            ...
            if index != 0:
                if row_type == "step":
                    ...
                    if variabilization:
                        create_step(current_step_name, row['Title'], row["Body Text"], row["Answer"], row["answerType"], step_count, choices, "", variabilization=row["Variabilization"],latex=latex,verbosity=verbosity)
                    else:
                        create_step(current_step_name, row['Title'], row["Body Text"], row["Answer"], row["answerType"], step_count, choices, "",latex=latex,verbosity=verbosity)
        ...
        for index, row in question.iterrows():
            ...
            if index != 0:
                if row_type == "step":
                    ...
                    choices = type(row["mcChoices"]) == str and row["mcChoices"]
                    if variabilization:
                        step_file.write(create_step(current_step_name, row['Title'], row["Body Text"], row["Answer"], row["answerType"], step_count, choices, step_images, variabilization=row["Variabilization"],latex=latex,verbosity=verbosity))
                    else:
                        step_file.write(create_step(current_step_name, row['Title'], row["Body Text"], row["Answer"], row["answerType"], step_count, choices, step_images,latex=latex,verbosity=verbosity))
                    step_file.close()
